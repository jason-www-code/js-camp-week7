...
function getThisWeekOrders(orders) {
  ...
  const thisWeekOrders = orders.filter(
    (order) =>
      dayjs.unix(order.createdAt).isAfter(thisWeekStart) &&
      dayjs.unix(order.createdAt).isBefore(thisWeekEnd),
  );
...
function validateOrderUser(data) {
  ...
  const paymentRules = ["ATM", "Credit Card", "Apple Pay"];
  ...
  if (!paymentRules.includes(payment)) {
...
const OrderService = {
   ...
   */
  async fetchOrders() {
    const { apiPath, baseURL, token } = this;
